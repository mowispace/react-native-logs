...
test("When set without dateformat should output local dateformat", () => {
  var log = rnlogs.logger.createLogger({ transport: transport })
  var outputData = ""
  var storeLog = inputs => (outputData += inputs)
  console["log"] = jest.fn(storeLog)
  log.debug("message")
  var pattern = /\d\d:\d\d:\d\d\ \| DEBUG \| message$/;
  expect(outputData).toMatch(pattern)
})

test("When set transportOptions={dateFormat:'local'}, should output toLocalString dateformat", () => {
  var log = rnlogs.logger.createLogger({ transport: transport, transportOptions:{ dateFormat:'local' } })
  var outputData = ""
  var storeLog = inputs => (outputData += inputs)
  console["log"] = jest.fn(storeLog)
  log.debug("message")
